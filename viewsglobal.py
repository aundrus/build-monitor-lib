...
# Import ART test results utility and Django/custom helpers
...
def globalviewDemo(request):
    """
    Global Builds Summary view.
    Handles request validation, ART results, DB queries, and result formatting.
    ...
    # Get ART test results for current session
    art_results_dict = get_art_test_results(request)

    # SQL query for global builds summary
    query = """
